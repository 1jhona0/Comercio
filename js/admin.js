...
/*Actualizar cliente con ajax*/
    $('.button-UPCli').click(function() {
    var myId = $(this).val();
        $('#update-cliente form#'+myId).submit(function(e) {
             e.preventDefault();
             var informacion=$('#update-cliente form#'+myId).serialize();
             var metodo=$('#update-cliente form#'+myId).attr('method');
             var peticion=$('#update-cliente form#'+myId).attr('action');
             $.ajax({
                type: metodo,
                url: peticion,
                data:informacion,
